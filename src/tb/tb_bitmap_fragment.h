...
/** TBImageloader is a class used to load skin images. It can be implemented
        in any way the system wants, but the system has to provide a image
   loader
        capable of handling all image formats used in the skin. */
class TBImageLoader {
 public:
  /** Static method used to create an image loader. The system must implement
     this
          function and create an implementation of the TBImageLoader interface.
     */
  static TBImageLoader* CreateFromFile(const std::string& filename);
...
class TBBitmapFragmentMap {
 public:
  ...
  /** Initialize the map with the given size. The size should be a power of two
     since
...
class TBBitmapFragment {
 public:
  ...
  /** Return the height allocated to this fragment. This may be larger than
     Height() depending
          of the internal allocation of fragments in a map. It should rarely be
     used. */
...
class TBBitmapFragmentManager {
 public:
  ...
  /** Set to true if a 1px border should be added to new fragments so stretched
          drawing won't get filtering artifacts at the edges (default is
     disabled). */
  void SetAddBorder(bool add_border) { m_add_border = add_border; }
  bool GetAddBorder() const { return m_add_border; }

  /** Get the fragment with the given image filename. If it's not already
     loaded,
          it will be loaded into a new fragment with the filename as id.
          returns nullptr on fail. */
  TBBitmapFragment* GetFragmentFromFile(const std::string& filename,
                                        bool dedicated_map);
  ...
  /** Clear all loaded bitmaps and all created bitmap fragments and maps.
          After this call, do not keep any pointers to any TBBitmapFragment
     created
  ...
  /** Set the number of maps (TBBitmaps) this manager should be allowed to
     create.
  ...
  /** Get the amount (in percent) of space that is currently occupied by all
     maps
