...
/** Return true if the given number string is a float number.
        Should only be called when you've verified it's a number with
   is_number(). */
...
/** TBValue holds value of a specific type.
        In addition to NULL, string, float, integer, it may also contain an
   array
        of attributes (TBValueArray), or an object (derived from TBTypedObject).

        When getting the value as a different type from what it is, it may
   convert
        its internal representation to that type. Exceptions are for array and
        object, which will return 0 when getting as numbers, or "" or object
   name
...
class TBValue {
 public:
  ...
  /** Take over ownership of content of source_value.
          Note:	-If source_value has string or array that are set with
     SET_AS_STATIC, it will make new copies of those.
  ...
  /** Copy the content of source_value to this value.
          Note: This value will become TYPE_NULL if source_value holds an
     object. We can't copy objects. */
