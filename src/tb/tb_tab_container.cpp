...
TBTabContainer::TBTabContainer()
    : m_need_page_update(true), m_current_page(0), m_align(TB_ALIGN_TOP) {
...
void TBTabContainer::SetValue(int index) {
  ...
  for (; page && tab; page = page->GetNext(), tab = tab->GetNext(), index++) {
    ...
    page->SetVisibilility(active ? WIDGET_VISIBILITY_VISIBLE
...
void TBTabContainer::SetAlignment(TB_ALIGN align) {
  ...
  m_root_layout.SetLayoutOrder(reverse ? LAYOUT_ORDER_TOP_TO_BOTTOM
                                       : LAYOUT_ORDER_BOTTOM_TO_TOP);
  m_tab_layout.SetLayoutPosition(reverse ? LAYOUT_POSITION_RIGHT_BOTTOM
