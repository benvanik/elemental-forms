...
TBNodeRefTree::~TBNodeRefTree() { s_ref_trees.Remove(this); }

TBValue& TBNodeRefTree::GetValue(const char* request) {
  if (TBNode* node = m_node.GetNodeFollowRef(request)) return node->GetValue();
...
void TBNodeRefTree::InvokeChangeListenersInternal(const char* request) {
  TBLinkListOf<TBNodeRefTreeListener>::Iterator iter =
...
TBNode* TBNodeRefTree::FollowNodeRef(TBNode* node) {
  ...
  while (node->GetValue().IsString()) {
    ...
    if (!next_node) {
      TBDebugPrint(
          "TBNodeRefTree::ResolveNode - Node not found on request \"%s\"\n",
          node_str);
      break;
    }
    node = next_node;
    ...
    if (node->m_cycle_id != cycle_id)
      ...
      TBDebugPrint(
          "TBNodeRefTree::ResolveNode - Reference loop detected on request "
          "\"%s\" from node \"%s\"\n",
          node_str, node->GetValue().GetString());
      return start_node;
    }
