...
TBSkinCondition::TBSkinCondition(TARGET target, PROPERTY prop,
...
bool TBSkin::Load(const char* skin_file, const char* override_skin_file) {
  if (!LoadInternal(skin_file)) return false;
  if (override_skin_file && !LoadInternal(override_skin_file)) return false;
  return ReloadBitmaps();
}

bool TBSkin::LoadInternal(const char* skin_file) {
  TBNode node;
  if (!node.ReadFile(skin_file)) return false;

  TBTempBuffer skin_path;
  if (!skin_path.AppendPath(skin_file)) return false;
  ...
  if (elements) {
    ...
    while (n) {
      ...
      while (TBNode* clone = n->GetNode("clone")) {
        ...
        if (clone_source) n->CloneChildren(clone_source);
      ...
      if (m_listener) m_listener->OnSkinElementLoaded(this, e, n);
  ...
  while (TBSkinElement* element = it.GetNextContent()) {
    if (!element->bitmap_file.empty()) {
      ...
      if (m_dim_conv.NeedConversion()) {
        ...
        if (element->bitmap) bitmap_dpi = m_dim_conv.GetDstDPI();
      }
...
TBSkin::~TBSkin() { g_renderer->RemoveListener(this); }
...
TBSkinElement* TBSkin::GetSkinElementStrongOverride(
    const TBID& skin_id, SKIN_STATE state,
  ...
  if (TBSkinElement* skin_element = GetSkinElement(skin_id)) {
    ...
    // state.
    TBSkinElementState* override_state =
        skin_element->m_strong_override_elements.GetStateElement(state,
                                                                 context);
    if (override_state) {
      if (TBSkinElement* override_element = GetSkinElementStrongOverride(
...
TBSkinElement* TBSkin::PaintSkin(const TBRect& dst_rect, const TBID& skin_id,
                                 SKIN_STATE state,
                                 TBSkinConditionContext& context) {
  return PaintSkin(dst_rect, GetSkinElement(skin_id), state, context);
}

TBSkinElement* TBSkin::PaintSkin(const TBRect& dst_rect, TBSkinElement* element,
                                 SKIN_STATE state,
                                 TBSkinConditionContext& context) {
  ...
  if (override_state) {
    if (TBSkinElement* used_override =
...
void TBSkin::PaintSkinOverlay(const TBRect& dst_rect, TBSkinElement* element,
                              SKIN_STATE state,
                              TBSkinConditionContext& context) {
...
void TBSkin::PaintElement(const TBRect& dst_rect, TBSkinElement* element) {
  ...
  else if (element->type == SKIN_ELEMENT_TYPE_STRETCH_IMAGE ||
           element->cut == 0)
...
TBRect TBSkin::GetFlippedRect(const TBRect& src_rect,
                              TBSkinElement* element) const {
...
void TBSkin::PaintElementBGColor(const TBRect& dst_rect,
...
void TBSkin::PaintElementStretchImage(const TBRect& dst_rect,
                                      TBSkinElement* element) {
  if (dst_rect.IsEmpty()) return;
  TBRect rect = dst_rect.Expand(element->expand, element->expand);
  TBRect src_rect = GetFlippedRect(
      TBRect(0, 0, element->bitmap->Width(), element->bitmap->Height()),
      element);
  g_renderer->DrawBitmap(rect, src_rect, element->bitmap);
}

void TBSkin::PaintElementStretchBox(const TBRect& dst_rect,
                                    TBSkinElement* element, bool fill_center) {
  if (dst_rect.IsEmpty()) return;
  ...
  if (element->flip_x) dst_cut_w = -dst_cut_w;
  if (element->flip_y) dst_cut_h = -dst_cut_h;

  // Corners
  g_renderer->DrawBitmap(TBRect(rect.x, rect.y, dst_cut_w, dst_cut_h),
                         TBRect(0, 0, cut, cut), element->bitmap);
  g_renderer->DrawBitmap(
      TBRect(rect.x + rect.w - dst_cut_w, rect.y, dst_cut_w, dst_cut_h),
      TBRect(bw - cut, 0, cut, cut), element->bitmap);
  g_renderer->DrawBitmap(
      TBRect(rect.x, rect.y + rect.h - dst_cut_h, dst_cut_w, dst_cut_h),
      TBRect(0, bh - cut, cut, cut), element->bitmap);
  g_renderer->DrawBitmap(
      TBRect(rect.x + rect.w - dst_cut_w, rect.y + rect.h - dst_cut_h,
             dst_cut_w, dst_cut_h),
  ...
  if (has_left_right_edges) {
    g_renderer->DrawBitmap(
        TBRect(rect.x, rect.y + dst_cut_h, dst_cut_w, rect.h - dst_cut_h * 2),
        TBRect(0, cut, cut, bh - cut * 2), element->bitmap);
    g_renderer->DrawBitmap(
        TBRect(rect.x + rect.w - dst_cut_w, rect.y + dst_cut_h, dst_cut_w,
               rect.h - dst_cut_h * 2),
  ...
  if (has_top_bottom_edges) {
    g_renderer->DrawBitmap(
        TBRect(rect.x + dst_cut_w, rect.y, rect.w - dst_cut_w * 2, dst_cut_h),
        TBRect(cut, 0, bw - cut * 2, cut), element->bitmap);
    g_renderer->DrawBitmap(
        TBRect(rect.x + dst_cut_w, rect.y + rect.h - dst_cut_h,
  ...
  if (fill_center && has_top_bottom_edges && has_left_right_edges)
    g_renderer->DrawBitmap(
        TBRect(rect.x + dst_cut_w, rect.y + dst_cut_h, rect.w - dst_cut_w * 2,
               rect.h - dst_cut_h * 2),
        TBRect(cut, cut, bw - cut * 2, bh - cut * 2), element->bitmap);
}

#ifdef TB_RUNTIME_DEBUG_INFO
void TBSkin::Debug() { m_frag_manager.Debug(); }
...
TBSkinElement::TBSkinElement()
    : bitmap(nullptr),
      cut(0),
      expand(0),
      type(SKIN_ELEMENT_TYPE_STRETCH_BOX),
      is_painting(false),
      is_getting(false),
      padding_left(0),
      padding_top(0),
      padding_right(0),
      padding_bottom(0),
      width(SKIN_VALUE_NOT_SPECIFIED),
      height(SKIN_VALUE_NOT_SPECIFIED),
      pref_width(SKIN_VALUE_NOT_SPECIFIED),
      pref_height(SKIN_VALUE_NOT_SPECIFIED),
      min_width(SKIN_VALUE_NOT_SPECIFIED),
      min_height(SKIN_VALUE_NOT_SPECIFIED),
      max_width(SKIN_VALUE_NOT_SPECIFIED),
      max_height(SKIN_VALUE_NOT_SPECIFIED),
...
int TBSkinElement::GetIntrinsicWidth() const {
  if (width != SKIN_VALUE_NOT_SPECIFIED) return width;
  if (bitmap) return bitmap->Width() - expand * 2;
  // FIX: We may want to check child elements etc.
  return SKIN_VALUE_NOT_SPECIFIED;
}

int TBSkinElement::GetIntrinsicHeight() const {
  if (height != SKIN_VALUE_NOT_SPECIFIED) return height;
  if (bitmap) return bitmap->Height() - expand * 2;
  // FIX: We may want to check child elements etc.
  return SKIN_VALUE_NOT_SPECIFIED;
}

void TBSkinElement::SetBitmapDPI(const TBDimensionConverter& dim_conv,
                                 int bitmap_dpi) {
  if (this->bitmap_dpi) {
...
bool TBSkinElement::HasState(SKIN_STATE state,
                             TBSkinConditionContext& context) {
  return m_override_elements.GetStateElement(
             state, context,
             TBSkinElementState::MATCH_RULE_ONLY_SPECIFIC_STATE) ||
...
bool TBSkinElementState::IsMatch(SKIN_STATE state,
                                 TBSkinConditionContext& context,
                                 MATCH_RULE rule) const {
...
bool TBSkinElementState::IsExactMatch(SKIN_STATE state,
                                      TBSkinConditionContext& context,
                                      MATCH_RULE rule) const {
  if (rule == MATCH_RULE_ONLY_SPECIFIC_STATE && this->state == SKIN_STATE_ALL)
    return false;
...
TBSkinElementState* TBSkinElementStateList::GetStateElement(
    SKIN_STATE state, TBSkinConditionContext& context,
  ...
  while (state_element) {
    if (state_element->IsMatch(state, context, rule)) return state_element;
    state_element = state_element->GetNext();
  }
  return nullptr;
}
...
TBSkinElementState* TBSkinElementStateList::GetStateElementExactMatch(
  ...
  TBSkinElementState* state_element = m_state_elements.GetFirst();
  while (state_element) {
    if (state_element->IsExactMatch(state, context, rule)) return state_element;
    state_element = state_element->GetNext();
  }
  return nullptr;
}
    ...
    for (TBNode* condition_node = element_node->GetFirstChild(); condition_node;
      ...
      if (strcmp(condition_node->GetName(), "state") == 0)
        ...
        TBSkinCondition::TARGET target =
            StringToTarget(condition_node->GetValueString("target", ""));

        const char* prop_str = condition_node->GetValueString("property", "");
        TBSkinCondition::PROPERTY prop = StringToProperty(prop_str);
        TBID custom_prop;
        if (prop == TBSkinCondition::PROPERTY_CUSTOM) custom_prop.Set(prop_str);
