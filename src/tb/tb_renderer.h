...
class TBRendererListener : public TBLinkOf<TBRendererListener> {
 public:
  ...
  /** Called when the context has been lost and all TBBitmaps need to be
     deleted.
          NOTE: Only do cleanup here. It's not safe to do work on any bitmap
     since the
          context is already lost. */
  virtual void OnContextLost() = 0;

  /** Called when the context has been restored again, and new TBBitmaps can be
     created
          again. */
...
class TBRenderer {
 public:
  ...
  /** Should be called before invoking paint on any widget.
          render_target_w and render_target_h should be the size of the render
     target
          that the renderer renders to. I.e window size, screen size or frame
     buffer object. */
