...
TBMessage::~TBMessage() { delete data; }

TBMessageHandler::TBMessageHandler() {}

TBMessageHandler::~TBMessageHandler() { DeleteAllMessages(); }
...
bool TBMessageHandler::PostMessageOnTime(TBID message, TBMessageData* data,
  ...
  if (TBMessage* msg = new TBMessage(message, data, fire_time, this)) {
    ...
    // we can
    // insert msg just before that. (Always keep the list ordered after fire
    // time)

    // NOTE: If another message is added during OnMessageReceived, it might or
    // might not be fired
    // in the right order compared to other delayed messages, depending on if
    // it's inserted before or
    ...
    // If we added it first and there's no normal messages, the next fire time
    // has
...
TBMessage* TBMessageHandler::GetMessageByID(TBID message) {
  ...
  while (TBMessage* msg = iter.GetAndStep())
    if (msg->message == message) return msg;
  return nullptr;
}

void TBMessageHandler::DeleteMessage(TBMessage* msg) {
  assert(msg->mh ==
         this);  // This is not the message handler owning the message!
...
void TBMessageHandler::DeleteAllMessages() {
  while (TBMessage* msg = m_messages.GetFirst()) DeleteMessage(msg);
}

// static
void TBMessageHandler::ProcessMessages() {
  // Handle delayed messages
  TBLinkListOf<TBMessageLink>::Iterator iter =
      g_all_delayed_messages.IterateForward();
  while (TBMessage* msg = static_cast<TBMessage*>(iter.GetAndStep())) {
    if (TBSystem::GetTimeMS() >= msg->fire_time_ms) {
      ...
      break;  // Since the list is sorted, all remaining messages should fire
              // later
  }
...
double TBMessageHandler::GetNextMessageFireTime() {
  ...
  if (g_all_delayed_messages.GetFirst()) {
    TBMessage* first_delayed_msg =
        static_cast<TBMessage*>(g_all_delayed_messages.GetFirst());
