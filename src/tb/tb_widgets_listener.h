...
/** TBWidgetListenerGlobalLink should never be created or subclassed anywhere
   except
...
class TBWidgetListener : public TBLinkOf<TBWidgetListener>,
                         public TBWidgetListenerGlobalLink {
 public:
  ...
  /** This is called when the widget request to be deleted.
          Return true if you want the widget to not die immediately, f.ex. to
     fade it out before it
          is deleted. If you return true, it's up to you to finally remove it
     from its parent delete it.

          Remember that the widget may still be deleted prematurely for many
     other reasons (f.ex if its parent is
          deleted or several listeners respond true and take on the task to
     delete it at some point). You can
  ...
  /** Called when the child is about to be removed from parent, after its
     parents OnChildRemove.
          Local listeners are invoked on the parent widget. */
  ...
  /** Called when a event is about to be invoked on a widget. This make it
     possible
          to intercept events before they are handled, and block it (by
     returning true).
