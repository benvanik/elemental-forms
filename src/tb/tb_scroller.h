...
class TBScrollerFunction {
 public:
  ...
  /** Calculate the distance reached at the given elapsed_time_ms with the given
   * start_speed. */
...
/** TBScrollerSnapListener may override the target scroll position of a
 * TBScroller. */
...
class TBScrollerSnapListener {
 public:
  ...
  /** Called when the target scroll position is calculated.
          ...
          target_x, target_y is the suggested target scroll position which may
     be changed
          to something else in this call.

          Note: The scroll positions are relative to the target widget (inner
     scrolled TBWidget).
                    If there's nested scrollable widgets, only the inner
     scrolled widget applies snapping. */
