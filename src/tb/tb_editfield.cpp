...
/** Get the delta that should be scrolled if dragging the pointer outside the
 * range min-max */
...
TBEditField::TBEditField()
  ...
  m_scrollbar_x.SetRect(
      TBRect(0, -scrollbar_x_h, -scrollbar_y_w, scrollbar_x_h));
...
TBRect TBEditField::GetVisibleRect() {
  ...
  if (m_scrollbar_y.GetOpacity()) rect.w -= m_scrollbar_y.GetRect().w;
  if (m_scrollbar_x.GetOpacity()) rect.h -= m_scrollbar_x.GetRect().h;
  return rect;
}
...
void TBEditField::SetStyling(bool styling) { m_style_edit.SetStyling(styling); }
...
void TBEditField::OnPaint(const PaintProps& paint_props) {
  ...
  TBRect old_clip;
  if (clip) old_clip = g_renderer->SetClipRect(visible_rect, true);
  ...
  if (m_style_edit.IsEmpty()) {
    ...
    TBRect placeholder_rect(visible_rect.x, visible_rect.y, visible_rect.w,
                            GetFont()->GetHeight());
  ...
  if (clip) g_renderer->SetClipRect(old_clip, false);
}
...
void TBEditField::OnPaintChildren(const PaintProps& paint_props) {
  ...
  DrawEdgeFadeout(
      GetVisibleRect(), TBIDC("TBEditField.fadeout_x"),
      TBIDC("TBEditField.fadeout_y"), m_scrollbar_x.GetValue(),
...
void TBEditField::OnFocusChanged(bool focused) { m_style_edit.Focus(focused); }
...
PreferredSize TBEditField::OnCalculatePreferredContentSize(
  ...
  if (m_adapt_to_content_size) {
    ...
    if (m_style_edit.packed.wrapping) {
      ...
      // the text
      // so we can get the actual content width with a constant result every
      // time.
  ...
  if (msg->message == TBIDC("blink")) {
    ...
    if (dx || dy)
      m_style_edit.MouseMove(
          TBPoint(pointer_move_widget_x, pointer_move_widget_y));
...
bool TBEditField::OnEnter() { return false; }

void TBEditField::Invalidate(const TBRect& rect) { TBWidget::Invalidate(); }

void TBEditField::DrawString(int32 x, int32 y, TBFontFace* font,
                             const TBColor& color, const char* str, int32 len) {
...
void TBTextFragmentContentWidget::UpdatePos(int x, int y) {
  m_widget->SetRect(
      TBRect(x, y, GetWidth(nullptr, nullptr), GetHeight(nullptr, nullptr)));
}

int32 TBTextFragmentContentWidget::GetWidth(TBFontFace* font,
                                            TBTextFragment* fragment) {
  return m_widget->GetRect().w ? m_widget->GetRect().w
                               : m_widget->GetPreferredSize().pref_w;
}

int32 TBTextFragmentContentWidget::GetHeight(TBFontFace* font,
                                             TBTextFragment* fragment) {
  return m_widget->GetRect().h ? m_widget->GetRect().h
                               : m_widget->GetPreferredSize().pref_h;
}

int32 TBTextFragmentContentWidget::GetBaseline(TBFontFace* font,
                                               TBTextFragment* fragment) {
