...
TBLinkListIterator::TBLinkListIterator(TBLinkList* linklist,
    ...
    : m_linklist(linklist), m_current_link(current_link), m_forward(forward) {
  Register();
}
...
TBLinkListIterator::~TBLinkListIterator() { Unregister(); }

void TBLinkListIterator::Register() {
  m_prev = nullptr;
  m_next = m_linklist->first_iterator;
  if (m_linklist->first_iterator) m_linklist->first_iterator->m_prev = this;
...
void TBLinkListIterator::Unregister() {
  ...
  if (m_linklist->first_iterator == this) m_linklist->first_iterator = m_next;
}
...
const TBLinkListIterator& TBLinkListIterator::operator=(
...
TBLink* TBLinkListIterator::GetAndStep() {
  if (!m_current_link) return nullptr;
...
void TBLinkListIterator::RemoveLink(TBLink* link) {
  ...
  if (m_current_link == link) GetAndStep();
}
...
TBLinkList::~TBLinkList() {
  ...
  while (first_iterator) first_iterator->UnregisterAndClear();
}
...
int TBLinkList::CountLinks() const {
  int count = 0;
  for (TBLink* link = first; link; link = link->next) count++;
  return count;
}
