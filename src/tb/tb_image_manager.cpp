...
void TBImageRep::IncRef() { ref_count++; }

void TBImageRep::DecRef() {
  ref_count--;
  if (ref_count == 0) {
    if (image_manager) image_manager->RemoveImageRep(this);
    delete this;
  }
}
...
bool TBImage::IsEmpty() const { return m_image_rep && m_image_rep->fragment; }
...
void TBImage::SetImageRep(TBImageRep* image_rep) {
  if (m_image_rep == image_rep) return;
...
TBImageManager::TBImageManager() { g_renderer->AddListener(this); }
...
void TBImageManager::OnContextLost() { m_frag_manager.DeleteBitmaps(); }
