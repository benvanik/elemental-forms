...
/** INFLATE_INFO contains info passed to TBWidget::OnInflate during resource
 * loading. */
struct INFLATE_INFO {
...
/** This macro creates a new TBWidgetFactory for the given class name so it can
        ...
        sync_type - The data type that should be synchronized through
   TBWidgetValue.
        add_child_z - The order in which children should be added to it by
   default.
...
#define TB_WIDGET_FACTORY(classname, sync_type, add_child_z)              \
  class classname##WidgetFactory : public TBWidgetFactory {               \
   public:                                                                \
    classname##WidgetFactory() : TBWidgetFactory(#classname, sync_type) { \
      Register();                                                         \
    }                                                                     \
...
/**
        ...
        and turn it into a hierarchy of widgets. It can create all types of
   widgets
        ...
        "@treename>noderequest". If treename is left out, the request will be
   looked
        ...
        Branches may be included or not depending on the value of a
   TBNodeRefTree
        node, using "@if @treename>noderequest" and optionally a following
   "@else".

        Branches may be included from TBNodeRefTree using
        "@include @treename>noderequest", or included from nodes specified
   previosly
        ...
        id					TBWidget::m_id
   TBID (string or int)
        group-id			TBWidget::m_group_id		TBID
   (string or int)
        value				TBWidget::SetValue
   integer
        data				TBWidget::m_data
   integer
        ...
        opacity				TBWidget::SetOpacity
   float (0 - 1)
        text				TBWidget::SetText
   string
        connection			TBWidget::Connect
   string
        axis				TBWidget::SetAxis
   x or y
        gravity				TBWidget::SetGravity
   string (combination of left, top, right, bottom, or all)
        visibility			TBWidget::SetVisibility
   string (visible, invisible, gone)
        state				TBWidget::SetState
   string (disabled)
        skin				TBWidget::SetSkinBg
   TBID (string or int)
        rect				TBWidget::SetRect
   4 integers (x, y, width, height)
        lp>width			TBWidget::SetLayoutParams
   dimension
        ...
        lp>height			TBWidget::SetLayoutParams
   dimension
