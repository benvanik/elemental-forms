...
class TBStyleEditListener {
 public:
  ...
  virtual void DrawString(int32 x, int32 y, TBFontFace* font,
                          const TBColor& color, const char* str,
                          int32 len = TB_ALL_TO_TERMINATION) = 0;
...
class TBTextOfs {
 public:
  ...
  void Set(TBBlock* new_block, int32 new_ofs) {
    block = new_block;
    ofs = new_ofs;
  }
...
class TBCaret {
 public:
  ...
  void SetGlobalOfs(int32 gofs, bool allow_snap = true,
                    bool snap_forward = false);
...
class TBBlock : public TBLinkOf<TBBlock> {
 public:
  ...
  int32 InsertText(int32 ofs, const char* text, int32 len,
  ...
  int32 CalculateStringWidth(TBFontFace* font, const char* str,
  ...
  void Invalidate();
  void BuildSelectionRegion(int32 translate_x, int32 translate_y,
...
class TBUndoRedoStack {
 public:
  ...
  TBUndoEvent* Commit(TBStyleEdit* styledit, int32 gofs, int32 len,
...
class TBStyleEdit {
 public:
  ...
  void InsertText(const char* text, int32 len = TB_ALL_TO_TERMINATION,
                  bool after_last = false, bool clear_undo_redo = false);
  void AppendText(const char* text, int32 len = TB_ALL_TO_TERMINATION,
 ...
 public:
  ...
  union {
    struct {
      ...
      uint32 calculate_content_width_needed : 1;  ///< If content_width needs to
      /// be updated next
      /// GetContentWidth-
      uint32 lock_scrollbars_counter : 5;  ///< Incremental counter for if
