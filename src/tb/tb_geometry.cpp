...
TBRegion::~TBRegion() { RemoveAll(true); }
...
bool TBRegion::GrowIfNeeded() {
  if (m_num_rects == m_capacity) {
    ...
    if (m_rects) memmove(new_rects, m_rects, sizeof(TBRect) * m_capacity);
    delete[] m_rects;
    m_rects = new_rects;
  ...
  for (int i = 0; i < m_num_rects; i++) {
    if (include_rect.Intersects(m_rects[i])) {
      ...
      for (int j = 0; j < inclusion_region.m_num_rects; j++) {
        if (!IncludeRect(inclusion_region.m_rects[j])) return false;
      }
      return true;
    }
  }
...
bool TBRegion::ExcludeRect(const TBRect& exclude_rect) {
  ...
  for (int i = 0; i < num_rects_to_check; i++) {
    if (m_rects[i].Intersects(exclude_rect)) {
      ...
      if (!AddExcludingRects(rect, exclude_rect, true)) return false;
    }
  }
  return true;
}

bool TBRegion::AddExcludingRects(const TBRect& rect, const TBRect& exclude_rect,
                                 bool coalesce) {
  ...
  if (remove.y > rect.y)
    if (!AddRect(TBRect(rect.x, rect.y, rect.w, remove.y - rect.y), coalesce))
      return false;
  if (remove.x > rect.x)
    if (!AddRect(TBRect(rect.x, remove.y, remove.x - rect.x, remove.h),
                 coalesce))
      return false;
  if (remove.x + remove.w < rect.x + rect.w)
    if (!AddRect(TBRect(remove.x + remove.w, remove.y,
                        rect.x + rect.w - (remove.x + remove.w), remove.h),
                 coalesce))
      return false;
  if (remove.y + remove.h < rect.y + rect.h)
    if (!AddRect(TBRect(rect.x, remove.y + remove.h, rect.w,
