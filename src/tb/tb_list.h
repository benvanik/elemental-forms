...
/** TBList is a list (array) of pointers to any kind of objects.
        ...
        You should use the typed TBListOf or TBListAutoDeleteOf for object
   storing! */
class TBListBackend {
 public:
...
class TBListOf {
 public:
  ...
  /** Remove the item at position index from the list and returns the pointer.
          This method should only be used when the order of the list is not
     important.
  ...
  void DeleteAll() {
    for (int i = 0; i < GetNumItems(); i++) delete (T*)Get(i);
    m_list.RemoveAll();
  }
  ...
  /** Search for the item with the given data and return the found index, or -1
   * if not found. */
  ...
  /** Get the capacity of the list number of items it can hold without
...
/** TBListAutoDeleteOf is a list (array) of pointers to the specified object
   type.
