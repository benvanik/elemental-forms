...
void DrawEdgeFadeout(const TBRect& dst_rect, TBID skin_x, TBID skin_y, int left,
  ...
  if (TBSkinElement* skin = g_tb_skin->GetSkinElement(skin_x)) {
    if (skin->bitmap) {
      ...
      if ((dw = GetFadeoutSize(left, bw)) > 0)
        g_renderer->DrawBitmap(TBRect(dst_rect.x, dst_rect.y, dw, dst_rect.h),
                               TBRect(0, 0, bw, bh), skin->bitmap);
      if ((dw = GetFadeoutSize(right, bw)) > 0)
        g_renderer->DrawBitmap(
            TBRect(dst_rect.x + dst_rect.w - dw, dst_rect.y, dw, dst_rect.h),
  ...
  if (TBSkinElement* skin = g_tb_skin->GetSkinElement(skin_y)) {
    if (skin->bitmap) {
      ...
      if ((dh = GetFadeoutSize(top, bh)) > 0)
        g_renderer->DrawBitmap(TBRect(dst_rect.x, dst_rect.y, dst_rect.w, dh),
                               TBRect(0, 0, bw, bh), skin->bitmap);
      if ((dh = GetFadeoutSize(bottom, bh)) > 0)
        g_renderer->DrawBitmap(
            TBRect(dst_rect.x, dst_rect.y + dst_rect.h - dh, dst_rect.w, dh),
