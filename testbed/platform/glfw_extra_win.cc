...
LRESULT PASCAL
TimerProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
...
void CreateTimerWindow() {
  ...
  wndclass.lpszMenuName = L"EL_TIMER_WINDOW";
  wndclass.lpszClassName = L"EL_TIMER_WINDOW";
  RegisterClass((WNDCLASS*)&wndclass);

  timer_hwnd = CreateWindow(L"EL_TIMER_WINDOW", L"EL_TIMER_WINDOW", 0, 10, 10,
                            100, 100, HWND_MESSAGE, (HMENU)NULL, NULL, NULL);
}
...
void glfwRescheduleTimer(unsigned int delay_ms) {
  if (!timer_hwnd) CreateTimerWindow();
