...
#define EL_USE_CURRENT_DIRECTORY
...
class ApplicationBackendGLFW : public ApplicationBackend {
 public:
  bool Init(TestbedApplication* app, int width, int height, const char* title);
...
static void timer_callback() {
  ...
  // RescheduleTimer.
  el::util::RescheduleTimer(MessageHandler::GetNextMessageFireTime());
}
#endif  // !EL_TARGET_LINUX

bool ApplicationBackendGLFW::Init(TestbedApplication* app, int width,
...
#if !defined(EL_USE_CURRENT_DIRECTORY) && defined(EL_TARGET_MACOSX)
...
void el::util::RescheduleTimer(uint64_t fire_time) {
  testbed::platform::ReschedulePlatformTimer(fire_time, false);
}
#endif  // !EL_TARGET_LINUX
...
// static
ApplicationBackend* ApplicationBackend::Create(TestbedApplication* app,
                                               int width, int height,
                                               const char* title) {
  auto backend = new platform::ApplicationBackendGLFW();
...
#ifdef EL_TARGET_WINDOWS

#define NOMINMAX
#include <mmsystem.h>
int PASCAL WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
                   LPSTR lpCmdLine, int nCmdShow) {
#if !defined(EL_USE_CURRENT_DIRECTORY)
